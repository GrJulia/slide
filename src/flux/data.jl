...
function get_dataloaders(config::Dict{String, Any})
    ...
    trainset = SparseDataset(train_data, train_labels, config["n_features"], config["n_classes"])
    testset = SparseDataset(test_data, test_labels, config["n_features"], config["n_classes"])
