...
const Signature = Vector{UInt32}

const EMPTY_SAMPLING =  Float32(-Inf) # TODO use const 'Float' defined in a different file
...
function initialize!(
    ...
    bin_idxs = @view perms[1:top_k, :]
    
    idxs_to_bins = [Vector{Int}() for _ = 1:data_len]

    # To each idx assign list of bins including this idx
    for bin_idx = 1:n_hashes
        @views for i in bin_idxs[:, bin_idx]
            idx = bin_idxs[i, bin_idx]
            push!(idxs_to_bins[idx], bin_idx)
    end

    # TODO sort each idx

    bins_offsets = [len(idx_to_bins) for idx_to_bins in idxs_to_bins]
    bins_offsets = acc_prefixes(bins_offsets)
    push!(bins_offsets, len(bins_offsets))

    idxs_to_bins = flatten(idxs_to_bins)
    ...
    DWTAHasher(idxs_to_bins, bins_offsets, n_hashes, log_n_hashes, rand_hash)
end
...
function signature(dwta::DWTAHasher, data::A)::Signature where {A<:AbstractVector{<:Number}}
    hashes = fill(EMPTY_SAMPLING, dwta.n_hashes)
    max_vals_in_bins = fill(EMPTY_SAMPLING, dwta.n_hashes)

    for idx in eachindex(data)
        idx_start, idx_end = dwta.bins_offsets[idx], dwta.bins_offsets[idx+1]
